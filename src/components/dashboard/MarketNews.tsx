...
// In a real application, this would come from props or a state management system
...
const NewsItem = ({ news }: { news: NewsItem }) => {
  const getImpactIcon = () => {
    switch (news.impact) {
      case 'positive':
        return <TrendingUp size={16} className="text-[#25C685]" />;
      case 'negative':
        return <TrendingDown size={16} className="text-[#FF5A5A]" />;
      default:
